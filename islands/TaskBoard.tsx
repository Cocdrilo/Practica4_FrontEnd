...
import { deleteTask, fetchTasks, moveTask, updateTaskStatus } from "../utils.ts";
...
export default function TaskBoard() {
  ...
  const handleMoveTask = async (taskId: string, userId: string) => {
    try {
      isLoading.value = true;
      error.value = null;
      ...
      const updatedStatus = await updateTaskStatus(taskId, status);
        console.log(updatedStatus)

      tasks.value = tasks.value.map((task) =>
          task._id === taskId ? { ...task, status: updatedStatus.data.status } : task
      );
    } catch (err) {
